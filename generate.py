...
if len(sys.argv) > 1: # The first option in argv, of course, is the name of the program itself.
	...
	for opt,args in opts:
		...
		if opt in ('-h', '--help'):
			log_it('INFO: -h or --help called, printing usage message')
			print_usage()
			sys.exit()
		elif opt in ('-v', '--verbose'):
			verbosity_level += 1
			log_it('INFO: -v or --verbose invoked, added one to verbosity level\n     Verbosity level is now ' + str(verbosity_level))
		elif opt in ('-q', '--quiet'):
			log_it('INFO: -q or --quiet invoked, decreasing verbosity level by one\n     Verbosity level is about to drop to ' + str(verbosity_level-1))
			verbosity_level -= 1
